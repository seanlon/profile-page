 ...
 */
define( 'WPPB_POTX_STATUS_SILENT', 0 );

/**
 * Drupal message based status reports.
 */
define( 'WPPB_POTX_STATUS_MESSAGE', 1 );
 ...
 */
define( 'WPPB_POTX_STATUS_STRUCTURED', 3 );
 ...
 */
function _wppb_potx_process_file ( $file_path, $strip_prefix = 0, $save_callback = '_wppb_potx_save_string', $version_callback = '_wppb_potx_save_version', $api_version = WPPB_POTX_API_6 )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;
    ...
    _wppb_potx_find_version_number ( $code, $file_name, $version_callback );
    ...
    $_wppb_potx_tokens = array();
    $_wppb_potx_lookup = array();
    $token_number = 0;
    $line_number = 1;
        ...
        if ( ( !is_array ( $token ) ) || ( ( $token[ 0 ] != T_WHITESPACE ) && ( $token[ 0 ] != T_INLINE_HTML ) ) ) {
            if ( is_array ( $token ) ) {
                ...
                if ( $token[ 0 ] == T_STRING || ( $token[ 0 ] == T_VARIABLE && in_array ( $token[ 1 ], $src_tokens ) ) ) {
                    if ( !isset( $_wppb_potx_lookup[ $token[ 1 ] ] ) ) {
                        $_wppb_potx_lookup[ $token[ 1 ] ] = array();
                    }
                    $_wppb_potx_lookup[ $token[ 1 ] ][ ] = $token_number;
                }
            }
            $_wppb_potx_tokens[ ] = $token;
            $token_number++;
        }
    ...
    if ( !empty( $src_tokens ) )
        foreach ( $src_tokens as $tk ) {
            _wppb_potx_find_t_calls_with_context ( $file_name, $save_callback, $tk );
        }

}

/**
 * Creates complete file strings with _wppb_potx_store()
 *
 * @param $string_mode
 *   Strings to generate files for: WPPB_POTX_STRING_RUNTIME or WPPB_POTX_STRING_INSTALLER.
 * @param $build_mode
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_build_files ( $string_mode = WPPB_POTX_STRING_RUNTIME, $build_mode = WPPB_POTX_BUILD_SINGLE, $force_name = 'general', $save_callback = '_wppb_potx_save_string', $version_callback = '_wppb_potx_save_version', $header_callback = '_wppb_potx_get_header', $template_export_langcode = NULL, $translation_export_langcode = NULL, $api_version = WPPB_POTX_API_6 )
{
    global $_wppb_potx_store;
    ...
    foreach ( $strings as $string => $string_info ) {
        foreach ( $string_info as $context => $file_info ) {
            ...
            if ( $build_mode == WPPB_POTX_BUILD_SINGLE ) {
                ...
                $file_name = ( $build_mode == WPPB_POTX_BUILD_CORE ? 'general' : str_replace ( '.info', '.module', $file_name ) );
            ...
            if ( strpos ( $string, "\0" ) !== FALSE ) {
                ...
                if ( isset( $translation_export_langcode ) ) {
                    $output .= _wppb_potx_translation_export ( $translation_export_langcode, $singular, $plural, $api_version );
                } else {
                    $output .= "msgstr[0] \"\"\n";
                    $output .= "msgstr[1] \"\"\n";
                }
                ...
                if ( isset( $translation_export_langcode ) ) {
                    $output .= _wppb_potx_translation_export ( $translation_export_langcode, $string, NULL, $api_version );
                } else {
                    $output .= "msgstr \"\"\n";
                }
            }
            ...
            if ( !isset( $_wppb_potx_store[ $file_name ] ) ) {
                $_wppb_potx_store[ $file_name ] = array(
                ...
                $_wppb_potx_store[ $file_name ][ 'sources' ] = array_unique ( array_merge ( $_wppb_potx_store[ $file_name ][ 'sources' ], $file_list ) );
                $_wppb_potx_store[ $file_name ][ 'strings' ] .= $output;
                $_wppb_potx_store[ $file_name ][ 'count' ] += 1;
            }
        }
    }
}
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_translation_export ( $translation_export_langcode, $string, $plural = NULL, $api_version = WPPB_POTX_API_6 )
{
    ...
    $language_column = $api_version > WPPB_POTX_API_5 ? 'language' : 'locale';
    $language_table = $api_version > WPPB_POTX_API_5 ? 'languages' : 'locales_meta';
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_get_header ( $file, $template_export_langcode = NULL, $api_version = WPPB_POTX_API_6 )
{
    // We only have language to use if we should export with that langcode.
    $language = NULL;
    if ( isset( $template_export_langcode ) ) {
        $language = db_fetch_object ( db_query ( $api_version > WPPB_POTX_API_5 ? "SELECT language, name, plurals, formula FROM {languages} WHERE language = '%s'" : "SELECT locale, name, plurals, formula FROM {locales_meta} WHERE locale = '%s'", $template_export_langcode ) );
    }
 ...
 */
function _wppb_potx_write_files ( $http_filename = NULL, $content_disposition = 'inline' )
{
    global $_wppb_potx_store;

    // Generate file lists and output files.
    if ( is_array ( $_wppb_potx_store ) ) {
        foreach ( $_wppb_potx_store as $file => $contents ) {
 ...
 */
function _wppb_potx_format_quoted_string ( $str )
{
    $quo = substr ( $str, 0, 1 );
    $str = substr ( $str, 1, -1 );
    if ( $quo == '"' ) {
 ...
 */
function _wppb_potx_marker_error ( $file, $line, $marker, $ti, $error, $docs_url = NULL )
{
    global $_wppb_potx_tokens;

    $tokens = '';
    $ti += 2;
    $tc = count ( $_wppb_potx_tokens );
    $par = 1;
    while ( ( ( $tc - $ti ) > 0 ) && $par ) {
        if ( is_array ( $_wppb_potx_tokens[ $ti ] ) ) {
            $tokens .= $_wppb_potx_tokens[ $ti ][ 1 ];
        } else {
            $tokens .= $_wppb_potx_tokens[ $ti ];
            if ( $_wppb_potx_tokens[ $ti ] == "(" ) {
                $par++;
            } else if ( $_wppb_potx_tokens[ $ti ] == ")" ) {
                $par--;
            }
        }
        $ti++;
    }
    wppb_potx_status ( 'error', $error, $file, $line, $marker . '(' . $tokens, $docs_url );
}
 ...
 */
function wppb_potx_status ( $op, $value = NULL, $file = NULL, $line = NULL, $excerpt = NULL, $docs_url = NULL )
{
    static $mode = WPPB_POTX_STATUS_CLI;
    ...
    switch ( $op ) {
        ...
        case 'status':
            ...
            $location_info = '';
            if ( ( $mode != WPPB_POTX_STATUS_STRUCTURED ) && isset( $file ) ) {
                if ( isset( $line ) ) {
                    if ( isset( $excerpt ) ) {
            ...
            $read_more = '';
            if ( ( $mode != WPPB_POTX_STATUS_STRUCTURED ) && isset( $docs_url ) ) {
                $read_more = ( $mode == WPPB_POTX_STATUS_CLI ) ? t ( 'Read more at @url', array( '@url' => $docs_url ) ) : t ( 'Read more at <a href="@url">@url</a>', array( '@url' => $docs_url ) );
            }

            // Error message or progress text to display.
            switch ( $mode ) {
                case WPPB_POTX_STATUS_MESSAGE:
                    drupal_set_message ( join ( ' ', array( $value, $location_info, $read_more ) ), $op );
                    break;
                case WPPB_POTX_STATUS_CLI:
                    if ( defined ( 'STDERR' ) && defined ( 'STDOUT' ) ) {
                        fwrite ( $op == 'error' ? STDERR : STDOUT, join ( "\n", array( $value, $location_info, $read_more ) ) . "\n\n" );
                    }
                    break;
                case WPPB_POTX_STATUS_SILENT:
                    if ( $op == 'error' ) {
                        $messages[ ] = join ( ' ', array( $value, $location_info, $read_more ) );
                    }
                    break;
                case WPPB_POTX_STATUS_STRUCTURED:
                    if ( $op == 'error' ) {
 ...
 * @param $string_mode
 *   String mode to use: WPPB_POTX_STRING_INSTALLER, WPPB_POTX_STRING_RUNTIME or
 *   WPPB_POTX_STRING_BOTH.
 */
function _wppb_potx_find_t_calls ( $file, $save_callback, $function_name = 't', $string_mode = WPPB_POTX_STRING_RUNTIME )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    // Lookup tokens by function name.
    if ( isset( $_wppb_potx_lookup[ $function_name ] ) ) {
        foreach ( $_wppb_potx_lookup[ $function_name ] as $ti ) {
            list( $ctok, $par, $mid, $rig ) = array( $_wppb_potx_tokens[ $ti ], $_wppb_potx_tokens[ $ti + 1 ], $_wppb_potx_tokens[ $ti + 2 ], $_wppb_potx_tokens[ $ti + 3 ] );
            ...
            if ( $par == "(" ) {
                if ( in_array ( $rig, array( ")", "," ) )
                    ...
                    $save_callback( _wppb_potx_format_quoted_string ( $mid[ 1 ] ), POTX_CONTEXT_NONE, $file, $line, $string_mode );
                } else {
                    // $function_name() found, but inside is something which is not a string literal.
                    _wppb_potx_marker_error ( $file, $line, $function_name, $ti, t ( 'The first parameter to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name ) ), 'http://drupal.org/node/322732' );
                }
            }
        }
    }
 ...
 *   WPPB_POTX_STRING_BOTH.
 */
function _wppb_potx_find_t_calls_with_context ( $file, $save_callback, $function_name = '_e', $string_mode = WPPB_POTX_STRING_RUNTIME )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;
            ...
            if ( $par == "(" ) {
                if ( in_array ( $rig, array( ")", "," ) )
                    ...
                    if ( $rig == ',' ) {
                        ...
                        /*$context = _wppb_potx_find_context($ti, $ti + 4, $file, $function_name);*/
                        ...
                        if ( $function_name == '_x' || $function_name == '_ex' ) {
                            ...
                            $text_plural = $_wppb_potx_tokens[ $ti + 4 ][ 1 ];
                        ...
                        if ( !isset( $_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ] ) ) return false;

                        if ( !preg_match ( '#^(\'|")(.+)#', $_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ] ) ) {
                            $constant_val = @constant ( $_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ] );
                            if ( !is_null ( $constant_val ) ) {
                                $domain = $constant_val;
                            } else {
                                if ( function_exists ( $_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ] ) ) {
                                    $domain = @$_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ]();
                                    if ( empty( $domain ) ) {
                                        return false;
                                    }
                                } else {
                            ...
                            $domain = trim ( $_wppb_potx_tokens[ $ti + $domain_offset ][ 1 ], "\"' " );
                        }

                        // exception for gettext calls with contexts
                        if ( false !== $context_offset ) {
                            if ( !preg_match ( '#^(\'|")(.+)#', $_wppb_potx_tokens[ $ti + $context_offset ][ 1 ] ) ) {
                                $constant_val = @constant ( $_wppb_potx_tokens[ $ti + $context_offset ][ 1 ] );
                                if ( !is_null ( $constant_val ) ) {
                                    $context = $constant_val;
                                } else {
                                    if ( function_exists ( $_wppb_potx_tokens[ $ti + $context_offset ][ 1 ] ) ) {
                                        $context = @$_wppb_potx_tokens[ $ti + $context_offset ][ 1 ]();
                                        if ( empty( $context ) ) {
                                            return false;
                                        }
                                    } else {
                                ...
                                $context = trim ( $_wppb_potx_tokens[ $ti + $context_offset ][ 1 ], "\"' " );
                            }
                    ...
                    if ( $domain !== POTX_CONTEXT_ERROR ) {
                        ...
                        $save_callback( _wppb_potx_format_quoted_string ( $mid[ 1 ] ), $domain, @strval ( $context ), $file, $line, $string_mode );
                        if ( isset( $text_plural ) ) {
                            $save_callback( _wppb_potx_format_quoted_string ( $text_plural ), $domain, $context, $file, $line, $string_mode );
                        }
                    }
                } else {
                    // $function_name() found, but inside is something which is not a string literal.
 ...
 *   Callback function used to save strings.
 */
function _wppb_potx_find_watchdog_calls ( $file, $save_callback )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    // Lookup tokens by function name.
    if ( isset( $_wppb_potx_lookup[ 'watchdog' ] ) ) {
        foreach ( $_wppb_potx_lookup[ 'watchdog' ] as $ti ) {
            list( $ctok, $par, $mtype, $comma, $message, $rig ) = array( $_wppb_potx_tokens[ $ti ], $_wppb_potx_tokens[ $ti + 1 ], $_wppb_potx_tokens[ $ti + 2 ], $_wppb_potx_tokens[ $ti + 3 ], $_wppb_potx_tokens[ $ti + 4 ], $_wppb_potx_tokens[ $ti + 5 ] );
            list( $type, $string, $line ) = $ctok;
            if ( $par == '(' ) {
                ...
                if ( in_array ( $rig, array( ')', ',' ) ) && $comma == ','
                    ...
                    $save_callback( _wppb_potx_format_quoted_string ( $mtype[ 1 ] ), POTX_CONTEXT_NONE, $file, $line );
                    $save_callback( _wppb_potx_format_quoted_string ( $message[ 1 ] ), POTX_CONTEXT_NONE, $file, $line );
                } else {
                    // watchdog() found, but inside is something which is not a string literal.
                    _wppb_potx_marker_error ( $file, $line, 'watchdog', $ti, t ( 'The first two watchdog() parameters should be literal strings. There should be no variables, concatenation, constants or even a t() call there.' ), 'http://drupal.org/node/323101' );
                }
            }
        }
    }
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_find_format_plural_calls ( $file, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    if ( isset( $_wppb_potx_lookup[ 'format_plural' ] ) ) {
        foreach ( $_wppb_potx_lookup[ 'format_plural' ] as $ti ) {
            list( $ctok, $par1 ) = array( $_wppb_potx_tokens[ $ti ], $_wppb_potx_tokens[ $ti + 1 ] );
            list( $type, $string, $line ) = $ctok;
            if ( $par1 == "(" ) {
                // Eat up everything that is used as the first parameter
                $tn = $ti + 2;
                $depth = 0;
                while ( !( $_wppb_potx_tokens[ $tn ] == "," && $depth == 0 ) ) {
                    if ( $_wppb_potx_tokens[ $tn ] == "(" ) {
                        $depth++;
                    } elseif ( $_wppb_potx_tokens[ $tn ] == ")" ) {
                        $depth--;
                    }
                    $tn++;
                }
                // Get further parameters
                list( $comma1, $singular, $comma2, $plural, $par2 ) = array( $_wppb_potx_tokens[ $tn ], $_wppb_potx_tokens[ $tn + 1 ], $_wppb_potx_tokens[ $tn + 2 ], $_wppb_potx_tokens[ $tn + 3 ], $_wppb_potx_tokens[ $tn + 4 ] );
                if ( ( $comma2 == ',' ) && ( $par2 == ')' || ( $par2 == ',' && $api_version > WPPB_POTX_API_5 ) ) &&
                    ...
                    if ( $par2 == ',' && $api_version > WPPB_POTX_API_6 ) {
                        // If there was a comma after the plural, we need to look forward
                        // to try and find the context.
                        $context = _wppb_potx_find_context ( $ti, $tn + 5, $file, 'format_plural' );
                    }
                    if ( $context !== POTX_CONTEXT_ERROR ) {
                        // Only save if there was no error in context parsing.
                        $save_callback(
                            _wppb_potx_format_quoted_string ( $singular[ 1 ] ) . "\0" . _wppb_potx_format_quoted_string ( $plural[ 1 ] ),
                            $context,
                            $file,
                            $line
                        );
                    ...
                    _wppb_potx_marker_error ( $file, $line, "format_plural", $ti, t ( 'In format_plural(), the singular and plural strings should be literal strings. There should be no variables, concatenation, constants or even a t() call there.' ), 'http://drupal.org/node/323072' );
                }
            }
        }
    }
 ...
 *   Callback function used to save strings.
 */
function _wppb_potx_find_perm_hook ( $file, $filebase, $save_callback )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    if ( isset( $_wppb_potx_lookup[ $filebase . '_perm' ] ) ) {
        ...
        if ( $filebase == 'node' ) {
            $line = $_wppb_potx_tokens[ $_wppb_potx_lookup[ 'node_perm' ][ 0 ] ][ 2 ];
            ...
            $count = 0;
            foreach ( $_wppb_potx_lookup[ $filebase . '_perm' ] as $ti ) {
                $tn = $ti;
                while ( is_array ( $_wppb_potx_tokens[ $tn ] ) || $_wppb_potx_tokens[ $tn ] != '}' ) {
                    if ( is_array ( $_wppb_potx_tokens[ $tn ] ) && $_wppb_potx_tokens[ $tn ][ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                        // hook_perm() is only ever found on a Drupal system which does not
                        // support context.
                        $save_callback( _wppb_potx_format_quoted_string ( $_wppb_potx_tokens[ $tn ][ 1 ] ), POTX_CONTEXT_NONE, $file, $_wppb_potx_tokens[ $tn ][ 2 ] );
                        $count++;
                    }
                    $tn++;
                }
 ...
 */
function _wppb_potx_find_end_of_function ( $here )
{
    global $_wppb_potx_tokens;

    // Seek to open brace.
    while ( is_array ( $_wppb_potx_tokens[ $here ] ) || $_wppb_potx_tokens[ $here ] != '{' ) {
        $here++;
    }
    $nesting = 1;
    while ( $nesting > 0 ) {
        $here++;
        if ( !is_array ( $_wppb_potx_tokens[ $here ] ) ) {
            if ( $_wppb_potx_tokens[ $here ] == '}' ) {
                $nesting--;
            }
            if ( $_wppb_potx_tokens[ $here ] == '{' ) {
                $nesting++;
 ...
 */
function _wppb_potx_skip_args ( $here )
{
    global $_wppb_potx_tokens;
    ...
    while ( !( ( $_wppb_potx_tokens[ $here ] == ',' && $nesting == 0 ) ||
        ( $_wppb_potx_tokens[ $here ] == ')' && $nesting == -1 ) ) ) {
        $here++;
        if ( !is_array ( $_wppb_potx_tokens[ $here ] ) ) {
            if ( $_wppb_potx_tokens[ $here ] == ')' ) {
                $nesting--;
 ...
 */
function _wppb_potx_find_context ( $tf, $ti, $file, $function_name )
{
    global $_wppb_potx_tokens;

    // Start from after the comma and skip the possible arguments for the function
    // so we can look for the context.
    if ( ( $ti = _wppb_potx_skip_args ( $ti ) ) && ( $_wppb_potx_tokens[ $ti ] == ',' ) ) {
        ...
        echo "TI:" . $ti . "\n";
        list( $com, $arr, $par ) = array( $_wppb_potx_tokens[ $ti ], $_wppb_potx_tokens[ $ti + 1 ], $_wppb_potx_tokens[ $ti + 2 ] );
        if ( $com == ',' && $arr[ 1 ] == 'array' && $par == '(' ) {
            ...
            while ( !( ( is_array ( $_wppb_potx_tokens[ $ti ] ) && ( in_array ( $_wppb_potx_tokens[ $ti ][ 1 ], array( '"context"', "'context'" ) ) ) && ( $_wppb_potx_tokens[ $ti ][ 0 ] == T_CONSTANT_ENCAPSED_STRING ) && ( $nesting == 0 ) ) ||
                ( $_wppb_potx_tokens[ $ti ] == ')' && $nesting == -1 ) ) ) {
                $ti++;
                if ( !is_array ( $_wppb_potx_tokens[ $ti ] ) ) {
                    if ( $_wppb_potx_tokens[ $ti ] == ')' ) {
                        $nesting--;
            ...
            if ( $nesting == 0 ) {
                ...
                list( $arw, $str ) = array( $_wppb_potx_tokens[ $ti + 1 ], $_wppb_potx_tokens[ $ti + 2 ] );
                if ( is_array ( $arw ) && $arw[ 1 ] == '=>' && is_array ( $str ) && $str[ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                    return _wppb_potx_format_quoted_string ( $str[ 1 ] );
                } else {
                    list( $type, $string, $line ) = $_wppb_potx_tokens[ $ti ];
                    // @todo: fix error reference.
                    _wppb_potx_marker_error ( $file, $line, $function_name, $tf, t ( 'The context element in the options array argument to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name ) ), 'http://drupal.org/node/322732' );
                    // Return with error.
 ...
 *   Callback function used to save strings.
 */
function _wppb_potx_find_menu_hook ( $file, $filebase, $save_callback )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    if ( isset( $_wppb_potx_lookup[ $filebase . '_menu' ] ) && is_array ( $_wppb_potx_lookup[ $filebase . '_menu' ] ) ) {
        // We have a menu hook in this file.
        foreach ( $_wppb_potx_lookup[ $filebase . '_menu' ] as $ti ) {
            $end = _wppb_potx_find_end_of_function ( $ti );
            $tn = $ti;
            while ( $tn < $end ) {
                // Look through the code until the end of the function.
                if ( $_wppb_potx_tokens[ $tn ][ 0 ] == T_CONSTANT_ENCAPSED_STRING && in_array ( $_wppb_potx_tokens[ $tn ][ 1 ], array( "'title'", '"title"', "'description'", '"description"' ) ) && $_wppb_potx_tokens[ $tn + 1 ][ 0 ] == T_DOUBLE_ARROW ) {
                    if ( $_wppb_potx_tokens[ $tn + 2 ][ 0 ] == T_CONSTANT_ENCAPSED_STRING ) {
                        // Menu items support no context.
                        $save_callback(
                            _wppb_potx_format_quoted_string ( $_wppb_potx_tokens[ $tn + 2 ][ 1 ] ),
                            POTX_CONTEXT_NONE,
                            $file,
                            $_wppb_potx_tokens[ $tn + 2 ][ 2 ]
                        );
                        $tn += 2; // Jump forward by 2.
                    } else {
                        wppb_potx_status ( 'error', t ( 'Invalid menu %element definition found in %hook. Title and description keys of the menu array should be literal strings.', array( '%element' => $_wppb_potx_tokens[ $tn ][ 1 ], '%hook' => $filebase . '_menu()' ) ), $file, $_wppb_potx_tokens[ $tn ][ 2 ], NULL, 'http://drupal.org/node/323101' );
                    }
                }
                $tn++;
            }
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_find_language_names ( $file, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    global $_wppb_potx_tokens, $_wppb_potx_lookup;

    foreach ( $_wppb_potx_lookup[ $api_version > WPPB_POTX_API_5 ? '_locale_get_predefined_list' : '_locale_get_iso639_list' ] as $ti ) {
        // Search for the definition of _locale_get_predefined_list(), not where it is called.
        if ( $_wppb_potx_tokens[ $ti - 1 ][ 0 ] == T_FUNCTION ) {
            break;
        }
    }
    ...
    while ( $ti < $end ) {
        while ( $_wppb_potx_tokens[ $ti ][ 0 ] != T_ARRAY ) {
            if ( !is_array ( $_wppb_potx_tokens[ $ti ] ) && $_wppb_potx_tokens[ $ti ] == ';' ) {
        ...
        $save_callback( _wppb_potx_format_quoted_string ( $_wppb_potx_tokens[ $ti ][ 1 ] ), POTX_CONTEXT_NONE, $file, $_wppb_potx_tokens[ $ti ][ 2 ] );
    }
}
 ...
 */
function _wppb_potx_find_version_number ( $code, $file, $version_callback )
{
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_add_date_strings ( $file, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    for ( $i = 1; $i <= 12; $i++ ) {
        $stamp = mktime ( 0, 0, 0, $i, 1, 1971 );
        if ( $api_version > WPPB_POTX_API_6 ) {
            // From Drupal 7, long month names are saved with this context.
            $save_callback( date ( "F", $stamp ), 'Long month name', $file );
        } elseif ( $api_version > WPPB_POTX_API_5 ) {
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_add_format_interval_strings ( $file, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    $components = array(
        '1 year' => '@count years',
        '1 week' => '@count weeks',
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_add_default_region_names ( $file, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    $regions = array(
        'left' => 'Left sidebar',
        'right' => 'Right sidebar',
 ...
 *   Drupal API version to work with.
 */
function _wppb_potx_find_info_file_strings ( $file_path, $file_name, $save_callback, $api_version = WPPB_POTX_API_6 )
{
    $info = array();

    if ( file_exists ( $file_path ) ) {
        $info = $api_version > WPPB_POTX_API_5 ? drupal_parse_info_file ( $file_path ) : parse_ini_file ( $file_path );
    }
 ...
 */
function _wppb_potx_parse_js_file ( $code, $file, $save_callback )
{
    $js_string_regex = '(?:(?:\'(?:\\\\\'|[^\'])*\'|"(?:\\\\"|[^"])*")(?:\s*\+\s*)?)+';
    ...
    if ( isset( $t_matches ) && count ( $t_matches ) ) {
        foreach ( $t_matches as $match ) {
            ...
            $save_callback( _wppb_potx_parse_js_string ( $match[ 1 ] ), POTX_CONTEXT_NONE, $file, 0 );
        }
    }
    ...
    if ( isset( $plural_matches ) && count ( $plural_matches ) ) {
        foreach ( $plural_matches as $index => $match ) {
            ...
            // @todo: figure out how to parse out context, once Drupal supports it.
            $save_callback(
                _wppb_potx_parse_js_string ( $match[ 1 ] ) . "\0" . _wppb_potx_parse_js_string ( $match[ 2 ] ),
                POTX_CONTEXT_NONE,
                $file,
                0
            );
 ...
 */
function _wppb_potx_parse_js_string ( $string )
{
    return _wppb_potx_format_quoted_string ( implode ( '', preg_split ( '~(?<!\\\\)[\'"]\s*\+\s*[\'"]~s', $string ) ) );
}
 ...
 */
function _wppb_potx_explore_dir ( $path = '', $basename = '*', $api_version = WPPB_POTX_API_6 )
{
    ...
    if ( $api_version > WPPB_POTX_API_5 ) {
        $extensions[ ] = 'js';
    }
    $files = array();
    ...
    if ( is_array ( $dirs ) ) {
        foreach ( $dirs as $dir ) {
            if ( !preg_match ( "!(^|.+/)(CVS|.svn|.git)$!", $dir ) ) {
                $files = array_merge ( $files, _wppb_potx_explore_dir ( "$dir/", $basename ) );
            }
        }
    }
 ...
 */
function _wppb_potx_save_version ( $value = NULL, $file = NULL )
{
    global $_wppb_potx_versions;

    if ( isset( $value ) ) {
        $_wppb_potx_versions[ $file ] = $value;
    } else {
        return $_wppb_potx_versions;
    }
}
 ...
 * @param $string_mode
 *   String mode: WPPB_POTX_STRING_INSTALLER, WPPB_POTX_STRING_RUNTIME
 *   or WPPB_POTX_STRING_BOTH.
 */
function _wppb_potx_save_string ( $value = NULL, $context = NULL, $file = NULL, $line = 0, $string_mode = WPPB_POTX_STRING_RUNTIME )
{
    global $_wppb_potx_strings, $_wppb_potx_install;
	
	if ( isset( $value ) ) {
        switch ( $string_mode ) {
            case WPPB_POTX_STRING_BOTH:
                ...
                $_wppb_potx_install[ $value ][ $context ][ $file ][ ] = $line . ' (dup)';
            // Break intentionally missing.
            case WPPB_POTX_STRING_RUNTIME:
                // Mark runtime strings as duplicates of installer strings if
                // the string was both recorded in the runtime and in the installer.
                $_wppb_potx_strings[ $value ][ $context ][ $file ][ ] = $line . ( $string_mode == WPPB_POTX_STRING_BOTH ? ' (dup)' : '' );
                break;
            case WPPB_POTX_STRING_INSTALLER:
                $_wppb_potx_install[ $value ][ $context ][ $file ][ ] = $line;
                break;
        }
    } else {
        return ( $string_mode == WPPB_POTX_STRING_RUNTIME ? $_wppb_potx_strings : $_wppb_potx_install );
    }
}
