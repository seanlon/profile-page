...
class PBLE_Import {
	...
	public function json_to_db( $json_content ) {
		...
		if ( $imported_array_from_json !== NULL ) {
			...
			$this->import_messages[$this->j]['message'] = __( 'Uploaded file is not valid json!', 'pble' );
		...
		if( isset( $_POST['pble-import'] ) ) {
			if( ! empty( $_FILES['pble-upload']['tmp_name'] ) ) {
				...
				if( empty( $this->import_messages ) ) {
					$this->import_messages[$this->j]['message'] = __( 'Import successfully!', 'pble' ) . "</p><p>" . __( 'Page will refresh in 3 seconds...', 'pble' ) . '<META HTTP-EQUIV="refresh" CONTENT="3">';
					$this->import_messages[$this->j]['type'] = 'updated';
					$this->j++;
					flush_rewrite_rules( false );
				}
			} else {
				$this->import_messages[$this->j]['message'] = __( 'Please select a .json file to import!', 'pble' );
				$this->import_messages[$this->j]['type'] = 'error';
				$this->j++;
			}
		}
